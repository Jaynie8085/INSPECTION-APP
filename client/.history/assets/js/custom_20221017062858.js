...
document.addEventListener("DOMContentLoaded", () => {
  function fetchBOoki() {
    fetch("http://localhost:3000/bookings")
      .then((resp) => resp.json())
      .then((data) => renderBOoki(data));
  }
  function renderBOoki(data) {
    for (const q of data) {
      ...
      function deleteQuote() {
        const url = `http://localhost:3000/BOoki/${q.id}`;
  ...
  function renderVehicles(data) {
    for (const q of data) {
      ...
      tbody.append(id, numberplate, chasisnumber, model, username, clear1);
      table.append(tbody);
      //Attach all the necessary attributes to delete button
      dislikesBtn.innerHTML = "Delete";
      // dislikesBtn.className = "btn-danger"; //for styling
