...
function inspections() {
  ...
  axios
    ...
    .then((result) => {
      // Signed in
      alert(`Success!! Transaction has been saved to our database!`, 1000);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderInspections(data) {
    for (const q of data) {
      //Find the container where we attach everything to
      const quoteUL = document.querySelector("#inspections-list");
      //Create all necessary elements
      const quoteLi = document.createElement("li");
      const blockQuote = document.createElement("blockquote");
      const p = document.createElement("p");
      ...
      //  multiplier = parseInt(multiplier);
       ...
       doc.text();
       doc.setFontSize(16);
       doc.text(20, 30, "This is to certify that the vehicle numberplate: " + numberplate "Has been certified with Inspections app for a period of 1year as a psv");
